<template>
  ...
  async function getTransactions() {
    try {
      const response = await getDjangoAPI.get('/transactions/')
      let TransactionTemp: Device[] = []
      response.data.forEach((device: any) => {
        TransactionTemp.push(new Device(device.serial, device.MAC, device.cid, device.mid, device.store,
                                    device.department, device.tid, device.pstation, device.register,
                                    device.ip, device.labelid, device.itassetid,
                                    device.status, device.statusnotes))
      });
      devices.value = TransactionTemp
    } catch (error) {
      console.error(error)
    }
